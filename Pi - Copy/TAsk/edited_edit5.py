...
def arm_and_takeoff_nogps(TAltitude):
    ...
    while vehicle.mode.name is not "GUIDED_NOGPS":
        print("Mode not guided")
    ...
    while True:
        ...
        set_attitude(roll_angle = 0.0, pitch_angle = 0.0, yaw_rate = 0.0, thrust = pidout, duration = 0)
        time.sleep(0.1)
...
def hover(a):
    ...
    while vehicle.mode.name is not "POSHOLD":
        current_altitude,Baro = true_hgt()
        print ("Mode: %s" % (vehicle.mode.name))
        print ("Altitude left %s" % (TAltitude - current_altitude))
        print(Baro)
...
set_attitude(pitch_angle = -1, thrust = 0.5, duration = 5)
hover(10)
