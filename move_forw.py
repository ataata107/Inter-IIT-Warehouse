...
pid.setpoint = 1.0   #aTargetAltitude(m)
...
def arm_and_takeoff_nogps(TAltitude):
    ...
    while True:
        ...
        if(flag_1 == True):
            ...
            #value_2 is right or left
            if(value_2=='left'):
                set_attitude(roll_angle = -1, thrust = pidout, duration = 0)
            elif(value_2=='right'):
                set_attitude(roll_angle = 1, thrust = pidout, duration = 0)
            else:
                set_attitude(pitch_angle = 1, thrust = pidout, duration = 0)
            if (flag_2 ==True):
