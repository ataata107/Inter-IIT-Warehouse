...
def main(TAltitude):
    ...
    while True:
        ...
        print ("VelX %s % (current_vel_x))
        print ("VelY %s % (current_vel_y))
        
        pidout_T = pid_T.compute_output(current_altitude)
        pidout_T += baseThrust
        print ("Height = ", (current_altitude), "px, ", "Thrust = ", pidout)
        pidout_R = pid_R.compute_output(current_vel_x)
        pidout_R += baseRoll
        pidout_P = pid_P.compute_output(current_vel_y)
        pidout_P += basePitch                
    
        
        #flag_1 is given that line is detected
        #flag_2 is given that intersection has reached

        if(flag_1 == True):
            if (current_offset_roll>=length/2 or current_offset_roll<=-length/2):
               set_attitude(roll_angle = pidout_R, thrust = pidout)
            else:
               set_attitude(pitch_angle = 1, thrust = pidout)
