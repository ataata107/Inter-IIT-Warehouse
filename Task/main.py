...
def arm_and_takeoff_nogps(TAltitude):
    ...
    while True:
        ...
        set_attitude(roll_angle = pidout_R, pitch_angle = -pidout_P, yaw_rate = 0.0, thrust = pidout_T, duration = 0)
        time.sleep(0.1)
...
def landing_nogps(aTargetAltitude):
    ...
    set_attitude(roll_angle = pidou, pitch_angle = -pidout_P, yaw_rate = 0.0, thrust = pidout_T, duration = 0)
    '''
    while True:
        current_altitude = vehicle.location.global_relative_frame.alt
        print(" Altitude: %s" % current_altitude)
