...
#       img = image
        ...
        if mark >2 :
                ...
                arearight = 0.0
                areabottom = 0.0
                if(top < len(contours) and right < len(contours) and bottom < len(contours) and cv2.contourArea(contours[top]) > 1 and cv2.contourArea(contours[right]) > 1 and cv2.contourArea(contours[bottom]) > 1):
                        tempL = []
                        tempM = []
                        tempO = []
                        src = []
                        ...
                        (thresh, warped) = cv2.threshold(warped, 230, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
#                       (thresh, warped) = cv2.threshold(warped, 230, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
                        warped = cv2.resize(warped,(100,100))
                        ...
                        imagez = zbar.Image(warped.shape[0],warped.shape[1],'GREY',warped.tostring())
                        scanner.scan(imagez)
